...
text_8 = visual.TextStim(win=win, name='text_8',
    text='Wir zeigen Ihnen ein Java Programm, dabei ist Ihre Aufgabe so schnell wie möglich für jede Klammer ( oder { einmal die Pfeiltaste nach unten zu drücken. Wenn Sie fertig sind, drücken Sie die Leertaste.\nAchtung! Wir suchen nur die offenen Klammern ( und {,\nandere Klammern, wie beispielsweise },), [, ] sollen ignoriert werden.',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
_key_resp_allKeys = []
# keep track of which components have finished
Hello_WorldComponents = [HelloWorldImage, key_resp, text_2, text_12]
...
# keep track of which components have finished
CrossfixationComponents = [image_7, key_resp_16]
...
thisExp.nextEntry()
# the Routine "Crossfixation" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
...
for thisTrial in trials:
    ...
    for thisLoop in Loop:
        ...
        if key_resp_16.keys in ['', [], None]:  # No response was made
            key_resp_16.keys = None
        Loop.addData('key_resp_16.keys',key_resp_16.keys)
        if key_resp_16.keys != None:  # we had a response
            Loop.addData('key_resp_16.rt', key_resp_16.rt)
        Loop.addData('key_resp_16.started', key_resp_16.tStartRefresh)
        Loop.addData('key_resp_16.stopped', key_resp_16.tStopRefresh)
        # the Routine "Crossfixation" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        for thisComponent in AskToContinueComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        Loop.addData('continuQuestion.started', continuQuestion.tStartRefresh)
        Loop.addData('continuQuestion.stopped', continuQuestion.tStopRefresh)
        # check responses
        if stop.keys in ['', [], None]:  # No response was made
            stop.keys = None
        Loop.addData('stop.keys',stop.keys)
        if stop.keys != None:  # we had a response
            Loop.addData('stop.rt', stop.rt)
        Loop.addData('stop.started', stop.tStartRefresh)
        Loop.addData('stop.stopped', stop.tStopRefresh)
