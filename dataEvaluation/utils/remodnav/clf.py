...
def deg_per_pixel(screen_size, viewing_distance, screen_resolution):
    ...
    """
    return degrees(atan2(.5 * screen_size, viewing_distance)) / (.5 * screen_resolution)
...
def find_peaks(vels, threshold):
    ...
    def _get_vels(start, end):
        current_vel = vels[start:end]
        current_vel = current_vel[~np.isnan(current_vel)]
        return current_vel
...
def find_movement_onsetidx(vels, start_idx, sac_onset_velthresh):
    idx = start_idx
    while idx > 0 and (vels[idx] > sac_onset_velthresh or vels[idx] <= vels[idx - 1]):
...
def find_movement_offset_idx(vels, start_idx, off_velthresh):
    idx = start_idx
    # shift saccade end index to the first element that is below the
    # velocity threshold
    while idx < len(vels) - 1 and (vels[idx] > off_velthresh or (vels[idx] > vels[idx + 1])):
        # we used to do this, but it could mean detecting very long
        # saccades that consist of (mostly) missing data
...
def find_pso_end(velocities, sac_vel_thresh, sac_peak_vel_thresh):
    pso_peaks = find_peaks(velocities, sac_peak_vel_thresh)
    pso_label = ''
    if pso_peaks:
        pso_label = 'HPSO'
    else:
        pso_peaks = find_peaks(velocities, sac_vel_thresh)
        if pso_peaks:
            pso_label = 'LPSO'
    if not pso_peaks:
        # no PSO
    ...
    pso_end = find_movement_offset_idx(velocities, pso_peaks[-1][1], sac_vel_thresh)
...
class EyeGazeClassifier(object):
    ...
    def __init__(self,
                 px2deg,
                 sampling_rate,
                 pursuit_vel_thresh=2.0,
                 noise_factor=5.0,
                 vel_thresh_start_velocity=300.0,
                 min_inter_saccade_duration=0.04,
        ...
        self.vel_thresh_start_vel = vel_thresh_start_velocity
        self.lp_cutoff_freq = lowpass_cutoff_freq
        self.pursuit_vel_thresh = pursuit_vel_thresh
        self.noise_factor = noise_factor

        for name, arg in [('min-intersaccade-duration', min_inter_saccade_duration),
                          ('min-saccade-duration', min_saccade_duration),
                          ('saccade-context-window-length', saccade_context_window_length),
        ...
        # convert to #samples
        self.min_intersac_dur = int(min_inter_saccade_duration * sr)
        self.min_sac_dur = int(min_saccade_duration * sr)
        self.sac_context_winlen = int(saccade_context_window_length * sr)
        self.max_pso_dur = int(max_pso_duration * sr)
        self.min_fix_dur = int(min_fixation_duration * sr)
        self.min_purs_dur = int(min_pursuit_duration * sr)
    ...
    def get_adaptive_saccade_velocity_velthresh(self, vels):
        ...
        tuple
          ...
          saccade onset.
        """
        cur_thresh = self.vel_thresh_start_vel
    ...
    def _detect_saccades(self, candidate_locs, data, start, end, context):
        ...
        for i, props in enumerate(sorted(
            ...
            sac_onset_velthresh = None
            sac_peak_velthresh = None
            if context:
            ...
            sacc_end = find_movement_offset_idx(
    ...
    def _classify_intersaccade_periods(self, data, start, end, saccade_events, saccade_detection):
    ...
    def _classify_intersaccade_period(self, data, start, end, saccade_detection):
        lgr.info('Determine NaN-free intervals in [%i:%i] (%i)', start, end, end - start)

        # split the ISP up into its non-NaN pieces:
        win_start = None
        for idx in range(start, end + 1):
            if win_start is None and idx < len(data) and not np.isnan(data['x'][idx]):
                win_start = idx
            elif win_start is not None and ((idx == end) or np.isnan(data['x'][idx])):
                for e in self._classify_intersaccade_period_helper(data, win_start, idx, saccade_detection):
                    yield e
                # reset non-NaN window start
                win_start = None

    def _classify_intersaccade_period_helper(self, data, start, end, saccade_detection):
        ...
        if length > (2 * self.min_intersac_dur) + self.min_sac_dur + self.max_pso_dur:
            lgr.info('Perform saccade detection in [%i:%i]', start, end)
            saccades = self._detect_saccades(None, data, start, end, context=None)
            saccade_events = []
                ...
                for s in saccades:
                    if kill_pso:
                        ...
                        if s['label'].endswith('PSO'):
                            continue
                    if s['start_time'] - start < self.min_intersac_dur or end - s['end_time'] < self.min_intersac_dur:
            ...
            if saccade_events:
                ...
                for e in self._classify_intersaccade_periods(data, start, end,
                                                             sorted(saccade_events, key=lambda x: x['start_time']),
    ...
    def _fix_or_pursuit(self, data, start, end):
        ...
        pursuit_peaks = find_peaks(win_vels, self.pursuit_vel_thresh)
        ...
        # loop over all peaks sorted by the sum of their velocities
        # i.e. longer and faster goes first
        for i, props in enumerate(sorted(pursuit_peaks, key=lambda x: x[2].sum(), reverse=True)):
            ...
            pursuit_start = find_movement_onsetidx(win_vels, pursuit_start, self.pursuit_vel_thresh)

            # move forward in time to find the pursuit offset
            pursuit_end = find_movement_offset_idx(win_vels, pursuit_end, self.pursuit_vel_thresh)
        ...
        for ev in merged_evs:
            ...
            estart = start + ev[1]
            eend = start + ev[-1]
            lgr.debug('Found %s [%i, %i]', label, estart, eend)

            # change of events or end
            yield self._mk_event_record(data, None, label, estart, eend)
    ...
    def preproc(self, data, min_blink_duration=0.02, dilate_nan=0.01, median_filter_length=0.05, savgol_length=0.019,
                savgol_polyord=2, max_vel=1000.0):
        """
        Parameters
        ----------
        ...
        if median_filter_length:
            ...
            med_velocities[1:] = (np.diff(median_filter(data['x'],
                                                        size=median_filter_length)) ** 2 + np.diff(
                median_filter(data['y'],
        ...
        arrs = [med_velocities]
        names = ['med_vel']
        arrs.extend([velocities, acceleration, data['x'], data['y']])
